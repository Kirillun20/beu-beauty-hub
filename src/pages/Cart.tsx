    ...
    return (
      <main className="pt-24 pb-20 container mx-auto px-4 text-center">
        <div className="max-w-md mx-auto py-20">
          ...
          <Link to="/catalog" className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-primary text-primary-foreground font-display font-semibold">
  ...
  return (
    <main className="pt-24 pb-20">
      <div className="container mx-auto px-4">
        <h1 className="font-display text-3xl md:text-4xl font-bold mb-8">Корзина</h1>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => {
              const { key, product, quantity, selectedVolume, unitPrice } = item;
              return (
                <motion.div key={key} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="flex gap-3 sm:gap-4 p-3 sm:p-4 glass-card rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link to={`/product/${product.id}`} className="font-display font-semibold text-sm hover:text-primary transition-colors line-clamp-2">{product.name}</Link>
                    {selectedVolume && (
                      <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] bg-primary/10 text-primary border border-primary/20">{selectedVolume}</span>
                    )}
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      <div className="flex items-center glass-card rounded-lg">
                        <button onClick={() => updateQuantity(key, quantity - 1)} className="p-1.5 text-muted-foreground hover:text-foreground"><Minus size={14} /></button>
                        <span className="px-3 text-sm font-semibold">{quantity}</span>
                        <button onClick={() => updateQuantity(key, quantity + 1)} className="p-1.5 text-muted-foreground hover:text-foreground"><Plus size={14} /></button>
                      </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="font-display font-bold text-sm sm:text-base">{(unitPrice * quantity).toFixed(2)}</p>
          ...
          <div className="glass-card rounded-xl p-5 sm:p-6 h-fit lg:sticky lg:top-24">
            ...
            <button onClick={clearCart} className="w-full mt-3 py-3 rounded-xl border border-border text-sm text-muted-foreground hover:text-foreground transition-colors">
              Очистить корзину
            </button>
          </div>
        </div>
